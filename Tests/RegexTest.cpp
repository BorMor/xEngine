...
{
	{
		...
		xMatchCollection matches = xRegex::Matches(str, "\\d{2}\\.\\d{2}\\.\\d{4}");
...

	{
		...
		xMatchCollection matches = xRegex::Matches(str, "\\b(?<word>\\w+)\\s+(\\k<word>)\\b", xRegexOptions::IgnoreCase);
		xCHECK(matches.Count() == 3);
